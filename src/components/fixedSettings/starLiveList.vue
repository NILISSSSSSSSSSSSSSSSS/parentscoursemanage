<template>
  <div class="all-user">
    <el-row class="tc mt20">
      <el-button @click='showEdit(0)' class="mb20" type='primary' icon='el-icon-plus' size='small'>新增大咖直播</el-button>
    </el-row>
    <!-- 搜索结果 -->
    ...
    <!-- 新增编辑弹框 -->
    <editStarLive :menuObj='menuObj' @closeMain="closeMain" @freshParent="searchList()" />
  </div>
</template>
<script>
  import editStarLive from "@/components/fixedSettings/editStarLive";
  import { bigCoffeeList, bigCoffeeDel, bigCoffeeOrder, bigCoffeeChangeSta } from '@/api/home'
  export default {
    components: {
      editStarLive
    },
      ...
      changeWeight(row, type) {
        ...
        if (type == 'up') {
          if (index - 1 < 0) {
            ...
            param.swapCourseLiveUuid = this.list[index - 1].uuid;
          }
        }
        else {
